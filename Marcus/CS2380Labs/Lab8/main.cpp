...
int main()
{
	...
	bh.go_to_url("http://expedia.com");
	bh.go_to_url("http://cars.com");
	bh.go_to_url("http://cheapOair.com");
	bh.go_to_url("http://bing.com");
	// History: [google.com, expedia.com, cars.com, cheapOair.com, (bing.com)]
	test(bh.current_url() == "http://bing.com");
	test(bh.can_go_back());
	test(!bh.can_go_forward());
	test(bh.past_url_count() == 4);
	test(bh.future_url_count() == 0);
	...
	test(bh.can_go_forward());
	test(bh.past_url_count() == 0);
	test(bh.future_url_count() == 4);

	bh.go_to_url("http://netflix.com");
	// History: [google.com, (netflix.com)]
